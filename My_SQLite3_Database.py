...
class Program_Ini():
        def write_geometry():
            path=os.path.join(DB_Path.get(), "My_SQLite3.ini")
        ...
        def change_language():
            path=os.path.join(DB_Path.get(), "My_SQLite3.ini")
            if Language.get()=="Spanish":title,msg="Cambiar Idioma","¡El Idioma se puede Cambiar después de que se cierre este Programa y luego se vuelva a abrir!"
            else:title,msg="Change Language","The Language Can Be Changed After This Program Is Closed Then Reopened!"
        ...
        def select_language():
            path=os.path.join(DB_Path.get(), "My_SQLite3.ini")
            config=configparser.ConfigParser()
            if not os.path.exists(path):config.write(open(path, 'w'))
            config.read(path)
            if not config.has_section('Language'):
                languages=["English","Spanish"]
                title,msg="You Are Here → Select Program Language","Please Select The Desired Program Language."
                lang=my_askstring(title, msg, languages)
                if lang=="" or lang==None:return
                else: Program_Ini.set_language(lang)
            else:# Set Language
                config=configparser.ConfigParser()
                config.read(path)
                try:
...
class Database_Ini:
    def __init__(self,funct,db_name,tbl_name=None,new_name=None,exist=None):
        ...
        try:
            ...
            if Language.get()=="Spanish":title,msg1,msg2='<Error de archivo INI '+funct+'>',funct+ ' '+self.table+'\n',f"\nError: '{ex}'"
            else:title,msg1,msg2='<INI File Error '+funct+'>',funct+ ' '+self.table+'\n',f"\nError: '{ex}'"
            msg=msg1+msg2
...
class Database():
    ...
    def create_connection(db_file):
        ...
        try:
            ...
            if Language.get()=="Spanish":title,msg1,msg2="<"+db_file+ "Conexión>",'Error Conectándose a '+db_file+'\n',f"\nError: '{er}'"
            else:title,msg1,msg2="<"+db_file+ "Connection>",'Error Connecting To '+db_file+'\n',f"\nError: '{er}'"
            msg=msg1+msg2
            messagebox.showerror(title, msg)
    def create_new_database():
        Database.destroy_widgets()
        if Language.get()=="Spanish":title,msg='Estás aquí → Crear Nueva Base de Datos','Ingrese un Nombre para esta Nuevo Base de Datos, luego haga clic en OK.' 
        else:title,msg="You Are Here → Create New Database",'Enter a Name For This New Database, Then Click OK.'        
        ...
        if not Database.database_exist(db_file):
            try:
                ...
                if Language.get()=="Spanish":title,title2,msg='<Crear Nueva Base de Datos>','Estás aquí → Nueva Base de Datos Creada → '+new_db,new_db+' Creado con éxito!'
                else:title,title2,msg='<Create New Database>',"You Are Here → New Database Created → "+new_db,new_db+' Created Successfully!'    
                messagebox.showinfo(title+" "+new_db, msg)
            ...
            if Language.get()=="Spanish":title,msg='<Crear Nueva Base de Datos> '+ new_db,'¡La Base de Datos ' +new_db+ ' ya existe!'
            else:title,msg='<Create New Database> '+ new_db,new_db+' Database Already Exist!'    
            messagebox.showinfo(title, msg)
        ...
        if num_tbls!=0:# Display 1st Table or Previous Table When Database Selected
            for name in tbls:
                if name[0]!="sqlite_sequence":
                    ...
                    break
            Database.exec_tbl_item(first_tbl)
    ...
    def import_database():
        try:
            ...
            else:title,msg="<Import Database>","Import Database Succeeded."
            messagebox.showinfo(title, msg)
            Active_Table.set("")
            populate_db_menu()
            set_default_colors()
    ...
    def create_new_table(tbl_name=None):
        ...
        if Edit_Definitions.get()==False:
            ...
            for c in range(num_colms):
                if c==0:
                    ...
                    if Language.get()=="Spanish":
                        txt="Nombre de Columna "+str(c-1)
                        entry_txt="Entrada "+ str(c-1) 
                    else:
                        txt="Column Name "+str(c-1)
                        entry_txt="Entry "+str(c-1)
                    colm_names.append(txt)
        ...
        try:
            ...
            Column_Defines=[]
            entry_count=0
            if Language.get()=="Spanish":btn_text="Adjuntar"
    ...
    def rename_table():
        ...
        try:
            ...
            if DB_Name.get()!="" and Active_Table.get()!="":  
                ...
                Database.populate_tbl_menu(DB_Path.get(),DB_Name.get()+".db3")                
        ...
        if Language.get()=="Spanish":
            ...
            for c in range(Num_Columns.get()-2):
                ...
                if Column_Defines[c].get()=="Data Type":
                    msg="Column "+str(c+1)+" Data Type Is Not Selected. Please Try Again!"
        ...
        try:
            ...
            for r, rows in enumerate(table_data):
                for c in range(colms):
                    ...
                    if str(rows[c])!="":
                        entry[entry_count].bind("<Button-3>", lambda event, text=str(rows[c]): copy_to_clipboard(text))
    ...
    def populate_column_names(list_of_names):
        try:
            ...
            messagebox.showerror(title, msg1+msg2)
            Database.destroy_widgets()
    def get_colm_data(table_name,column_name):
        try:
            colm_name=str(column_name).replace(" ","_")
            conn=sqlite3.connect(Active_DB.get())
            cursor=conn.cursor()
            column_data=[row[0] for row in cursor.execute("SELECT "+colm_name+" FROM "+table_name)]
            conn.close()
            return column_data# Returns as List
        except sqlite3.Error as error:
            if Language.get()=="English":messagebox.showerror("<Column "+column_name+" Information>", error)
            else:messagebox.showerror("<Columna "+column_name+" Información>", error)
            return
        finally:
            if conn:conn.close()
    def colm_agg(event,name):
        try:
            ...
            conn=sqlite3.connect(Active_DB.get())
            cursor=conn.cursor()
            column_data=Database.get_colm_data(Active_Table.get(),colm_name)
            cursor.execute("SELECT MAX(CAST("+colm_name+" AS FLOAT)) FROM "+Active_Table.get())
            colm_max=round(cursor.fetchone()[0],4)
            if Language.get()=="Spanish":colm_max=str(colm_max).replace(",",".")
            cursor.execute("SELECT MIN(CAST("+colm_name+" AS FLOAT)) FROM "+Active_Table.get())
            colm_min=round(cursor.fetchone()[0],4)
            cursor.execute("SELECT AVG(CAST("+colm_name+" AS FLOAT)) FROM "+Active_Table.get())
            colm_avg=round(cursor.fetchone()[0],4)
            cursor.execute("SELECT TOTAL(CAST("+colm_name+" AS FLOAT)) FROM "+Active_Table.get())
            colm_total=round(cursor.fetchone()[0],4)
            conn.close()
        except sqlite3.Error as error:
            if Language.get()=="English":messagebox.showerror("<Column "+name+" Information>", error)
            else:messagebox.showerror("<Columna "+name+" Información>", error)
            return
        finally:
            if conn:conn.close()
        if colm_total!=0.0:
    ...
    def create_default_db():
        set_default_colors()
        db_file=os.path.join(DB_Path.get(), "Example"+".db3")
        Active_DB.set(db_file)
        Num_Columns.set(7)
...
def copy_to_clipboard(txt):
    try:
        if Language.get()=="English":msg=' Copied To Clipboard.'
        else:msg=" Copiado en el Portapapeles."
        if txt!='':
            os.system("echo.%s|clip" % txt)
            clipboard_msg=Label(root, text='"'+txt+'"'+msg,background="#33FFFF", 
                foreground='#000000',highlightthickness=4,highlightbackground = "#FFFFFF", highlightcolor= "#FFFFFF",font=root.font)
            pixel_width=clipboard_msg.winfo_reqwidth()# Required Width For Text In Pixels
            clipboard_msg.place(x=(root.winfo_width()*0.5)-(0.5*pixel_width), y=0)# Center x
...
def populate_menus():
    if Language.get()=="English":
        ...
        file_menu.add_command(label='Change Language',command=lambda:Program_Ini.change_language())
        ...
        file_menu.add_command(label='Cambiar Idioma',command=lambda:Program_Ini.change_language())
        file_menu.add_separator()
...
if __name__ == '__main__':
    ...
    root.after(500, Program_Ini.select_language())
    Program_Ini.set_geometry()
    root.mainloop()
